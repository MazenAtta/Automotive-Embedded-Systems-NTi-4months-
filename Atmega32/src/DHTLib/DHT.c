...
enum DHT_Status_t DHT_ReadRaw(uint8_t Data[4])
{
	...
	{
		// Request data
		DIO_VidInitPin(DHT_Pin, OUTPUT); // DHT_PIN = Output

		DIO_VidWritePin(DHT_Pin, LOW); // DHT_PIN = 0
		_delay_ms(__DHT_Delay_Read);

		// Setup DHT_PIN as input with pull-up resistor so as to read data
		// DHT_PIN = 1 (Pull-up resistor)
		DIO_VidInitPin(DHT_Pin, INPLUP); // DHT_PIN = Input

		// Wait for response for 20-40us
		retries = 0;
		while (DIO_u8GetPinValue(DHT_Pin))
		{
			_delay_us(2);
			retries += 2;
			if (retries > 60)
		...
		for (i = 0; i < 5; i++)
			for (j = 7; j >= 0; j--)
			{
				...
				if (__DHT_STATUS == DHT_Ok)
				{
					...
					if (DIO_u8GetPinValue(DHT_Pin)) // If HIGH
